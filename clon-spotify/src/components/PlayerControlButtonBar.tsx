...
export function PlayerControlButtonBar() {
  ...
  return (
    <div className="flex justify-center flex-row flex-nowrap items-center gap-4">
      ...
      <button className="bg-white text-black rounded-full p-2 hover:scale-110" onClick={onPlayPause} title={isPlaying ? "Pausar":"Reproducir"} >
        {isPlaying ? <Pause className=""/> : <Play className=""/>}
      </button>
      <button className="hover:scale-110" onClick={onNextSong} title="Siguiente">
        <Next/>
      </button>
    </div>
  );
